...
def loadDataSet():
    filename = 'AirQualityUCI.txt'
    return np.loadtxt(filename, dtype=np.float)



def normGauss(x,mu,sigma):
    '''
...
def em(dataArray,k,mu,sigma,step = 10):
    ...
    n = len(k)       # 高斯分布个数
    # 数据个数
    dataNum = dataArray.size   # 数据的个数
    # 初始化gama数组
    gamaArray = np.zeros((n,dataNum))
    for s in range(step):
        for i in range(n):
            for j in range(dataNum):
                Sum = sum([k[t]*normGauss(dataArray[j],mu[t],sigma[t]) for t in range(n)])
                gamaArray[i][j] = k[i]*normGauss(dataArray[j],mu[i],sigma[i])/float(Sum)
        # 更新 mu
        for i in range(n):
